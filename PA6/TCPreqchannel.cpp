...
TCPRequestChannel::TCPRequestChannel(const string host, const string port)
{
	...
	else // client side because host is not empty
	{
		...
		hints.ai_socktype = SOCK_STREAM;
		int status;
		// getting address info of host
		if ((status = getaddrinfo(host.c_str(), port.c_str(), &hints, &res)) != 0)
		{
			cerr << "getaddrinfo: " << gai_strerror(status) << endl;
			exit(-1); // because constructor should not return
		}
...
{
	sockfd = fd; // to make a channel out of a socket file descriptor
}

TCPRequestChannel::~TCPRequestChannel()
{
	close(sockfd); // destructor just closes socket file descriptor
}

int TCPRequestChannel::cread(void *msgbuf, int bufcapacity)
{
	return recv(sockfd, msgbuf, bufcapacity, 0); // 4th arg is flags
}
