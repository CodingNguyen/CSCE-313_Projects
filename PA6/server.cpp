...
int main(int argc, char *argv[])
{
	...
	TCPRequestChannel *main_channel = new TCPRequestChannel("", port); // main socket to accept connections
	while (1) 
	{ // main accept() loop
		...
		// putting accepted connection into its own thread for processing
		TCPRequestChannel* client_channel = new TCPRequestChannel(client_socket);
		thread t(handle_process_loop, client_channel); // passing the channel instead of the socket
		t.detach();
	}
