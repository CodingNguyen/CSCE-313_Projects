...
void event_polling_thread(int n, int p, int w, int m, FIFORequestChannel *wchans[], BoundedBuffer *request_buffer, HistogramCollection *hc, int mb)
{
    char buf[1024];
    double response;
    char recvbuf[mb];
    ...
    for (int i = 0; i < w; i++)
    {
        ...
        nsent++;
        int rfd = wchans[i]->getrfd(); // add channel file descriptor to epoll list
